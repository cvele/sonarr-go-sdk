...
> FeedV3CalendarSonarrIcsGet(ctx).PastDays(pastDays).FutureDays(futureDays).TagList(tagList).Unmonitored(unmonitored).PremieresOnly(premieresOnly).AsAllDay(asAllDay).Execute()
...
func main() {
    ...
    resp, r, err := apiClient.CalendarFeedApi.FeedV3CalendarSonarrIcsGet(context.Background()).PastDays(pastDays).FutureDays(futureDays).TagList(tagList).Unmonitored(unmonitored).PremieresOnly(premieresOnly).AsAllDay(asAllDay).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `CalendarFeedApi.FeedV3CalendarSonarrIcsGet``: %v\n", err)
...
Other parameters are passed through a pointer to a apiFeedV3CalendarSonarrIcsGetRequest struct via the builder pattern
